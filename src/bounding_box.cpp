  ...
  scale_factor_(kScaleFactor)
{
 ...
 BoundingBox::BoundingBox(const cv::Point& TopLeft, const cv::Point& BottomDown)
     :scale_factor_(kScaleFactor)
 {
   // Set BoundingBox manually from 2 Point
   x1_ = TopLeft.x;
   y1_ = TopLeft.y;
   x2_ = BottomDown.x;
   y2_ = BottomDown.y;
...
void BoundingBox::Shift(const cv::Mat& image,
  ...
  while ((new_width < 0 || new_width > image.cols - 1) && num_tries_width < kMaxNumTries) {
    ...
    new_width = std::max(1.0, min(static_cast<double>(image.cols - 1), new_width));
    num_tries_width++;
  }
